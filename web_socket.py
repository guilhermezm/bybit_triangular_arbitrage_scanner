...
class WebSocketConnection:
            ...
            try:
                async with websockets.connect(WS_URL) as websocket:
                    subscribe_message = {
                        "req_id": self.pair,
                        "op": "subscribe",
                        "args": [f"orderbook.{self.depth}.{self.pair}"]
                    }
    ...
    async def process_message(self, message, filtered_combinations, initial_quantity):
        if 'op' in message:
            ...
            if 'data' in message:
                pair = message['data'].get('s')
                asks = [(Decimal(price), Decimal(quantity)) for price, quantity in message['data'].get('a', [])]
                bids = [(Decimal(price), Decimal(quantity)) for price, quantity in message['data'].get('b', [])]
