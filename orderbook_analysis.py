...
def execute_routes(filter_combinations: list, orderbook_data: dict, initial_quantity: Decimal) -> None:
    ...
    for combination in filter_combinations:
        ...
        if operation[0] == "B":
            A_B_price, A_quantity = calculate_effective_price(orderbook_data[a_symbol]['a'], efec_initial)
        elif operation[0] == "S":
            A_B_price, A_quantity = calculate_effective_price(orderbook_data[a_symbol]['b'], efec_initial)
